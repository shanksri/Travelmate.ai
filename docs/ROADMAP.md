...
| AviationStack, Google Places/Maps, Tavily Search | 🔄 **Flights are real** under `TRAVELMATE_PROVIDER=live` — but from **Travelpayouts** ([app/providers/travelpayouts.py](../app/providers/travelpayouts.py)), not AviationStack, because AviationStack has no fare data at all (schedules and status only). Lodging, attractions and weather are still the mock provider ([app/providers/mock.py](../app/providers/mock.py)). Real clients for AviationStack and Tavily exist and are tested ([aviationstack.py](../app/providers/aviationstack.py), [tavily.py](../app/providers/tavily.py)) but aren't used by the planner. No Google Places/Maps client exists yet. |
