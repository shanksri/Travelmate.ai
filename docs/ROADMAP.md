...
| AviationStack, Google Places/Maps, Tavily Search | 🔄 **Flights are real** under `TRAVELMATE_PROVIDER=live` — live Google Flights fares via SerpApi's hosted MCP server ([app/providers/google_flights.py](../app/providers/google_flights.py)), not AviationStack, because AviationStack has no fare data at all (schedules and status only). Travelpayouts was tried first and dropped: its cached fares were too sparse. Lodging, attractions and weather are still the mock provider ([app/providers/mock.py](../app/providers/mock.py)). Real clients for AviationStack and Tavily exist and are tested ([aviationstack.py](../app/providers/aviationstack.py), [tavily.py](../app/providers/tavily.py)) but aren't used by the planner. No Google Places/Maps client exists yet. |
| PostgreSQL long-term memory | ✅ `SqlTripStore` ([app/store.py](../app/store.py)) behind the same `TripStore` Protocol as the in-memory default; `docker-compose.yml` runs Postgres 16. Stores each trip version as one JSON row rather than a normalized schema — there's nothing to migrate when `PlannedTrip`'s shape changes. Now an **append-only version history**: every trip has a `thread_id` grouping its versions (1, 2, 3...), so a requested change appends rather than overwrites. See [README.md](../README.md#revisions-and-trip-history). |
| (not in the diagram) Revisions | ✅ [app/agent/reviser.py](../app/agent/reviser.py) — "more activities on day 3" against an existing trip re-runs **only** the itinerary step and saves the result as the next version, with the change recorded in `change_note`. Flights/lodging/dates carry over untouched and are withheld from the model's prompt entirely. Chosen over a LangGraph checkpointer: the feature needs versioned snapshots of a finished itinerary, not mid-graph resumability. |
