"""Turn the itinerary agent's raw JSON text into a validated `Itinerary`.
...
def parse_itinerary(raw_json: str, request: TripRequest) -> Itinerary:
    try:
    ...
    if len(itinerary.days) != expected_days:
        raise ItineraryValidationError(
            ...
            f"{expected_days} days, but you returned {len(itinerary.days)} entries in "
